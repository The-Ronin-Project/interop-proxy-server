...
class ParticipantService(private val practitionerService: PractitionerService) {
    val logger = KotlinLogging.logger {}

    fun getParticipants(ehrParticipants: Set<EHRParticipant>, tenant: Tenant): Map<EHRParticipant, ProxyParticipant> {
        logger.info("Retrieving ${ehrParticipants.size} Participants from Aidbox for Tenant ${tenant.mnemonic}")
        ...
        return ehrParticipants.associateWith { ehrParticipant ->
            ehrParticipant.toProxyServerParticipant(foundIDsMap, tenant)
        }
    }
}
