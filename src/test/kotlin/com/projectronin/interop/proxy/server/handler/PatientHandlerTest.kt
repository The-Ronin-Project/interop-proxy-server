...
class PatientHandlerTest {
    ...
    @Test
    fun `post search matching supports supplied given names with multiple words that is represented as a single given name by the EHR`() {
        val patient1 =
            mockk<R4Patient> {
                every { id } returns Id("Patient-UUID-1")
                every { identifier } returns
    ...
    fun `patientsByTenants - no patients found`() {
        ...
        every { ehrFactory.getVendorFactory(tenant1) } returns
            mockk {
                every { patientService } returns
                    mockk {
    ...
    fun `patientsByTenants - patients found for all`() {
        ...
        every { ehrFactory.getVendorFactory(tenant1) } returns
            mockk {
                every { patientService } returns
                    mockk {
                        every { findPatient(tenant1, LocalDate.of(2001, 2, 3), "Josh", "Smith") } returns
                            listOf(
                                patient1,
                            )
                    }
        ...
        every { ehrFactory.getVendorFactory(tenant2) } returns
            mockk {
                every { patientService } returns
                    mockk {
                        every { findPatient(tenant2, LocalDate.of(2001, 2, 3), "Josh", "Smith") } returns
                            listOf(
                                patient2,
                            )
                    }
